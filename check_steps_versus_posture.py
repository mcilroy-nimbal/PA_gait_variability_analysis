...
def plot_data(output_fname):

    final_df = pd.read_csv("W:\\Annotated posture\\testing_unbouted_posture\\"+output_fname)
    final_df = final_df[final_df['bouted'] != 'all']

    xvar = "step_count"

    g = sns.catplot(data=final_df, x=xvar, y='posture_group', hue="bouted", kind="boxen")
    #g = sns.catplot(data=final_df, x=xvar, y="posture_group", hue="bouted",kind="violin", bw_adjust=.5, cut=0, split=True,)

    g.set_axis_labels(xvar, "Posture")
    ...
    list = ['bouted','unbouted']
    #loop through the postures
    for i in list:
        df = final_df[final_df['bouted'] == i]
        g = sns.catplot(data=df, x=xvar, y="posture_group")
        #g = sns.swarmplot(data=df, x=xvar, y="posture_group", size=3)
        g.set_axis_labels(xvar, "Posture")
        g.set_titles(f"Group - {i}")
        for ax in g.axes.flat:
            ax.tick_params(axis='x', rotation=45)
            ax.set_xlim(0, 10000)
        plt.tight_layout()
...
def process_data(output_fname):
    ...
    output_path = Path(root + '\\Annotated posture\\testing_unbouted_posture\\' + output_fname)
...
process_data ('processed_0717')
#plot_data ('processed_0717')